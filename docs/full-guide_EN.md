...
- **Persistent AlphaSift navigation** - Screening always remains visible in the left navigation. The service is enabled by default; when explicitly disabled or unavailable, the page shows status and recovery guidance without automatically starting a market-wide scan.
...
| `/api/v1/alphasift/screen/tasks` | POST | Submit an AlphaSift screening background task (enabled by default; explicitly setting `ALPHASIFT_ENABLED=false` rejects execution) |
