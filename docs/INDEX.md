...
| 参与开发或提交 PR | [贡献指南](CONTRIBUTING.md) | [代码地图](architecture/code-map.md)、[API 规格](architecture/api_spec.json) |
...
| [投资画像与多策略对比](investor-profile.md) | `/profile` 访谈推荐与策略中心直选、画像存储语义、问股多策略对比（`AGENT_COMPARE_MAX`、`skill_breakdown`/`skill_consensus`）、降级与已知限制 |
