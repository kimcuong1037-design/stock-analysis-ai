...
- 🔎 **AlphaSift 选股固定入口** - 左侧“选股”始终可见；服务默认开启，显式关闭或依赖异常时页面会展示状态与修复指引，不会自动运行全市场扫描
...
| `/api/v1/alphasift/screen/tasks` | POST | 后台提交 AlphaSift 选股任务（默认开启；显式设置 `ALPHASIFT_ENABLED=false` 时拒绝执行） |
