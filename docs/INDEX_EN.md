...
| Contribute code or docs | [Contributing Guide (EN)](CONTRIBUTING_EN.md) | [Code Map](architecture/code-map.md) <sub><sub>![P2 Badge](https://img.shields.io/badge/P2-yellow?style=flat)</sub></sub> (Chinese-only), [API Spec](architecture/api_spec.json) |
...
| [Investor Profile And Multi-Strategy Comparison](investor-profile.md) <sub><sub>![P2 Badge](https://img.shields.io/badge/P2-yellow?style=flat)</sub></sub> (Chinese-only) | `/profile` interview recommendations and manual Strategy Center selection, profile storage semantics, multi-strategy chat comparison (`AGENT_COMPARE_MAX`, `skill_breakdown`/`skill_consensus`), degradation, and known limitations |
