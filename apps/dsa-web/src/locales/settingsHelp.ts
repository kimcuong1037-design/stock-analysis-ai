...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.ALPHASIFT_ENABLED': {
    title: 'AlphaSift 选股',
    summary: '控制是否启用内置 AlphaSift 选股服务；左侧“选股”入口始终可见。',
    usage: '默认开启。显式设为 false 可禁止策略读取和选股执行；Web 会继续显示入口与服务状态。若 alphasift.dsa_adapter 缺失，请执行 pip install -r requirements.txt 或重建后端产物。',
    valueNotes: ['AlphaSift 作为 DSA 后端依赖安装，/install 仅作为显式修复入口保留。', '选股结果仅用于研究辅助，不构成投资建议。'],
    impact: ['影响 AlphaSift 策略读取和选股 API，不影响 Web 选股入口的可见性。'],
    notes: ['AlphaSift 初筛候选，DSA 补充行情、基本面和新闻上下文；关闭时不影响原有分析、报告和通知流程。'],
  },
  'settings.data_source.ALPHASIFT_ENRICHMENT_MAX_WORKERS': {
    title: 'AlphaSift 候选增强并发数',
    summary: '控制最终候选补充行情、基本面和新闻时的最大并发数。',
    usage: '可设为 1 到 3，默认 3；设为 1 可回退到串行增强路径。',
    valueNotes: ['并发只作用于最多 3 个最终候选。', '输出顺序仍按 AlphaSift 原始 rank，不按请求完成顺序改变。'],
    impact: ['较高并发可缩短等待时间，也会同时占用更多数据源请求额度。'],
    notes: ['单个候选或数据源失败仍按部分成功语义返回，不会取消其它候选。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.ALPHASIFT_ENABLED': {
    ...
    summary: 'Controls the built-in AlphaSift screening service; Screening always remains visible in the left navigation.',
    usage: 'Enabled by default. An explicit false blocks strategy loading and screening execution while the Web app keeps the entry and service status visible. If alphasift.dsa_adapter is missing, run pip install -r requirements.txt or rebuild the backend artifact.',
    ...
    impact: ['Affects AlphaSift strategy loading and screening APIs, but not the visibility of the Web screening entry.'],
  ...
  'settings.data_source.ALPHASIFT_ENRICHMENT_MAX_WORKERS': {
    title: 'AlphaSift Enrichment Concurrency',
    summary: 'Controls concurrency while adding quote, fundamental, and news context to final candidates.',
    usage: 'Accepts 1 through 3 and defaults to 3. Set it to 1 to restore serial enrichment.',
    valueNotes: ['Concurrency applies only to the final Top 3 candidates.', 'Output remains ordered by the original AlphaSift rank, not completion order.'],
    impact: ['Higher concurrency can reduce waiting time while using more data-source capacity at once.'],
    notes: ['A candidate or provider failure keeps partial-success semantics and does not cancel other candidates.'],
  },
  'settings.data_source.ALPHASIFT_INSTALL_SPEC': {
